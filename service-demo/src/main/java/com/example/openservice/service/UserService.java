...
public class UserService {
    ...
    private static final Map<Long, User> MOCK_DB_DATA = new HashMap<>();
    ...
    public void addUser(User user) {
        ...
        user.setId(key);
        MOCK_DB_DATA.put(key, user);
    }

    public void updateUser(User user) {
        MOCK_DB_DATA.replace(user.getId(), user);
    }

    public void deleteUser(Long id) {
        MOCK_DB_DATA.remove(id);
    }

    public Collection<User> AllUser() {
        return new ArrayList<>(MOCK_DB_DATA.values());
    }

    public User user(Long id) {
        return MOCK_DB_DATA.get(id);
    }
}
